...
plot <- read_csv("Your_path/Fig3_Data_Epicurve_MBG.csv")
...
# Infections
ggplot() +
  geom_line(data = plot, aes(x = as.Date(day_begin), y = case_rate_ra_f, colour = MBG_C), linewidth = 1) +
  ...
  geom_rect(data = si_df, aes(xmin = start_date, xmax = end_date,
                              ymin = max(plot$case_rate_ra_f, na.rm = TRUE) + 0.05 * max(plot$case_rate_ra_f, na.rm = TRUE),
                              ymax = max(plot$case_rate_ra_f, na.rm = TRUE) + 2 * 0.05 * diff(range(plot$case_rate_ra_f, na.rm = TRUE)), 
                              fill = si)) +
  ...
  annotate("text", x = as.Date("2020-04-20"), y = max(plot$case_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$case_rate_ra_f, na.rm = TRUE), label = "Period 1",  colour = "black", size = 4.5, fontface = "bold") +
  annotate("text", x = as.Date("2020-09-15"), y = max(plot$case_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$case_rate_ra_f, na.rm = TRUE), label = "Period 2a", colour = "black", size = 4.5, fontface = "bold") +
  annotate("text", x = as.Date("2021-01-01"), y = max(plot$case_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$case_rate_ra_f, na.rm = TRUE), label = "Period 2b", colour = "black", size = 4.5, fontface = "bold") +
  annotate("text", x = as.Date("2021-04-18"), y = max(plot$case_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$case_rate_ra_f, na.rm = TRUE), label = "Period 3",  colour = "black", size = 4.5, fontface = "bold") +
  annotate("text", x = as.Date("2021-08-18"), y = max(plot$case_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$case_rate_ra_f, na.rm = TRUE), label = "Period 4",  colour = "black", size = 4.5, fontface = "bold") +
  annotate("text", x = as.Date("2021-11-20"), y = max(plot$case_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$case_rate_ra_f, na.rm = TRUE), label = "Period 5",  colour = "black", size = 4.5, fontface = "bold") 

# Save plot
ggname <- "Epi_MBG_Infs.eps"
#ggsave(ggname, width = 300, height = 150, units = "mm", dpi = 600)


# Hospitalisations
ggplot() +
  geom_line(data = plot, aes(x = day_begin, y = hosp_rate_ra_f, colour = MBG_C), linewidth = 1) +
  scale_x_date(date_breaks = "2 month", date_labels = "%b %Y",
               limits = c(as.Date("2019-12-31"), as.Date("2022-01-03")),
               expand = c(0, 0)) +
  ...
  geom_rect(data = si_df, aes(xmin = start_date, xmax = end_date,
                              ymin = max(plot$hosp_rate_ra_f, na.rm = TRUE) + 0.05 * max(plot$hosp_rate_ra_f, na.rm = TRUE),
                              ymax = max(plot$hosp_rate_ra_f, na.rm = TRUE) + 2 * 0.05 * diff(range(plot$hosp_rate_ra_f, na.rm = TRUE)), 
                              fill = si)) +
  scale_fill_gradient2(low = "white", mid = "tomato", high = "purple4", midpoint = (max(si_df$si)/2)) +
  geom_vline(xintercept = as.Date("2020-06-29"), colour = "black", linetype = 2, linewidth = 0.5) + 
  geom_vline(xintercept = as.Date("2020-11-30"), colour = "black", linetype = 2, linewidth = 0.5) +
  ...
  annotate("text", x = as.Date("2020-04-20"), y = max(plot$hosp_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$hosp_rate_ra_f, na.rm = TRUE), label = "Period 1",  colour = "black", size = 4.5, fontface = "bold") +
  annotate("text", x = as.Date("2020-09-15"), y = max(plot$hosp_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$hosp_rate_ra_f, na.rm = TRUE), label = "Period 2a", colour = "black", size = 4.5, fontface = "bold") +
  annotate("text", x = as.Date("2021-01-01"), y = max(plot$hosp_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$hosp_rate_ra_f, na.rm = TRUE), label = "Period 2b", colour = "black", size = 4.5, fontface = "bold") +
  annotate("text", x = as.Date("2021-04-18"), y = max(plot$hosp_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$hosp_rate_ra_f, na.rm = TRUE), label = "Period 3",  colour = "black", size = 4.5, fontface = "bold") +
  annotate("text", x = as.Date("2021-08-18"), y = max(plot$hosp_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$hosp_rate_ra_f, na.rm = TRUE), label = "Period 4",  colour = "black", size = 4.5, fontface = "bold") +
  annotate("text", x = as.Date("2021-11-20"), y = max(plot$hosp_rate_ra_f, na.rm = TRUE) + 0.030*max(plot$hosp_rate_ra_f, na.rm = TRUE), label = "Period 5",  colour = "black", size = 4.5, fontface = "bold") 


# Save plot
#ggname <- "Epi_MBG_Hosp.eps"
#ggsave(ggname, width = 300, height = 150, units = "mm", dpi = 600)
