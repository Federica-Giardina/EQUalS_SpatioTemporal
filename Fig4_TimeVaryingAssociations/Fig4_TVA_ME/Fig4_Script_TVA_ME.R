...
# ----- Write and/or import separte data ----- 
# Write resulting dataset to a .csv format
#write.csv(plot.me, paste0(format(Sys.Date(), "%Y%m%d"), "_MargEff_", type, ".csv"))



#plot <- read.csv("C:/Users/z710214/OneDrive - Radboudumc/z710214/My Documents/PhD/S5 - EQUalS/Writing (Carsten)/20250922_Final_export/Fig8_TVA_ME/Fig8_Data_TVA_ME_InfsAgg3.csv")
#plot <- read.csv("C:/Users/z710214/OneDrive - Radboudumc/z710214/My Documents/PhD/S5 - EQUalS/Writing (Carsten)/20250922_Final_export/Fig8_TVA_ME/Fig8_Data_TVA_ME_HospAgg3.csv")
#plot <- read.csv("C:/Users/z710214/OneDrive - Radboudumc/z710214/My Documents/PhD/S5 - EQUalS/Writing (Carsten)/20250922_Final_export/SupFig10_TVA_ME_SensAnalysis/SupFig10_Data_TVA_ME_HospAgg1.csv")
#plot <- read.csv("C:/Users/z710214/OneDrive - Radboudumc/z710214/My Documents/PhD/S5 - EQUalS/Writing (Carsten)/20250922_Final_export/SupFig10_TVA_ME_SensAnalysis/SupFig10_Data_TVA_ME_HospAgg5.csv")
...
# Plot results
ggplot(data = plot, aes(x = as.Date(date), y = q2*100, colour = level.r, fill = level.r, group = level.r)) + 
  
  geom_point(size = 0.1) + 
  geom_line(linewidth = .61, alpha     = .71) + 
  
  geom_ribbon(data = plot, aes(ymin = q1*100, ymax = q3*100, fill = level.r), colour  = NA, alpha = .3) +
  
  scale_colour_manual(values      = custom_palette) +
  scale_fill_manual(  values      = custom_palette) +
  
  scale_y_continuous( breaks       = seq(-0.5*100, 0.5*100, 0.00500*100), 
                      #minor_breaks = seq(-0.5*100, 0.5*100, 0.0005*100),
                      limits       = c(-0.0075*100, NA)        # -0.025- to 0.010 for INFS and -0.0075 for HOSPS!
  ) +
  ...
  labs(title  = "", x      = "", 
       y      = "Average Marginal Effect (%), Median and 95% CrI", 
       colour = "Area Type:",
       fill   = "Area Type:",
       #tag = "a"
       ) +
  ...
  annotate("text", x = as.Date("2020-04-20"), y = max(plot$q2)*100 + 0.2*max(plot$q2)*100, label = "Period 1",  colour = "black", fontface = "bold") +
  annotate("text", x = as.Date("2020-09-15"), y = max(plot$q2)*100 + 0.2*max(plot$q2)*100, label = "Period 2a", colour = "black", fontface = "bold") +
  annotate("text", x = as.Date("2021-01-01"), y = max(plot$q2)*100 + 0.2*max(plot$q2)*100, label = "Period 2b", colour = "black", fontface = "bold") +
  annotate("text", x = as.Date("2021-04-18"), y = max(plot$q2)*100 + 0.2*max(plot$q2)*100, label = "Period 3",  colour = "black", fontface = "bold") +
  annotate("text", x = as.Date("2021-08-18"), y = max(plot$q2)*100 + 0.2*max(plot$q2)*100, label = "Period 4",  colour = "black", fontface = "bold") +
  annotate("text", x = as.Date("2021-11-20"), y = max(plot$q2)*100 + 0.2*max(plot$q2)*100, label = "Period 5",  colour = "black", fontface = "bold") 
