...
HospBasic1$waic$p.eff
HospBasic2$waic$p.eff
HospBasic3$waic$p.eff
...
#SANITY CHECK!
## Easy method of MSPE
mean((Cases.nb.wk$hosp_obs - lgocv.hosp.agg.mob5.post$mean)^2)
## vs complicated method.
Table.LGOCV.hosp.post.agg.mob5
...
# These are the old definitions that are not entirely applicable to a binomial model.
## STANDARD MODEL COMPARISON METRICS
DIC <- function(x){
  data.frame(
...
table_sumstats <- round(do.call(rbind, lapply(list(
  res.hosp.basic,
  res.hosp.basic.mob,
  res.hosp.basic.mob.ts), DIC)), 6)
table_sumstats
...
cv_stats <- function(model){
  ...
  MAE  <- mean(abs(data$hosp_obs - cv_pred_counts))
  MSPE <- mean(   (data$hosp_obs - cv_pred_counts)^2)
  LS = -mean(log(model$cv))
...
# DEFINE LS AND MSPE FOR LOOCV & LGOCV
CV <- function(x){
  LS = -mean(log(x$cv))
  ...
  MSPE = mean((Cases.nb.wk$hosp_obs - expectation)^2)
